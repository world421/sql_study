...
CREATE VIEW view_emp AS (
    SELECT
    employee_id,
    -- first_name || ' ' || last_name AS full_name,-- 새롭게 만들어진데이터는 별칭을 붙여야함,
    job_id,
    salary
    FROM employees  
    WHERE department_id = 60
...
원본 테이블이 NOT NULL 인 경우 VIEW에INSERT가 불가능합니다.
VIEW에서 사용하는 컬럼이 가상열인 경우에도 안됩니다.
*/
SELECT * FROM view_emp_dept_jobs;

--!  fristname + lastname 합쳐져서 새롭게 만들어진 거라 인서트 진행할 수 없엉
-- 두번째 컬럼인 'full_name'은 가상열(virtual column)이기 때문에 INSERT 안됨)
...
-- 원본 테이블의 null을 허용하지 않는 컬럼떄문에 들어갈 수 없습니다
INSERT INTO view_emp
(employee_id, job_id, salary)
VALUES(300,'test',salart); -- 안됨!

-- ※ 삽입 ,수정, 삭제 성공시 원본테이블도 반영됩니다 ※
