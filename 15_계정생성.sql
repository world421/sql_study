...
SELECT ANY TABLE... -> 어떤 테이블도 조회할 수 있는 권한 
...
GRANT RESOURCE , CONNECT , DBA TO user1; -- 권한 줄 때는 TO 

REVOKE RESOURCE, CONNECT, DBA FROM user1; -- 권한 회수 할때는 FROM 
...
-- 현재 접속중인 계정은 삭제불가 ! 
-- 계정삭제할때 객체를 가지고있으면 삭제안해줌 
-- user1으로 table만들었으니까 table도 날라감 
--------------------------------------------
/*
...
QUOTA UNLIMITED ON user_tablespace;
 -- k,m,g,t, unlimited (용량 제한없음)

-- tablesplace 지우는 방법 (2가지)

-- 1) tablespace 내의 객체를 전체 삭제
DROP TABLESPACE user_tablespace INCLUDING CONTENTS;

-- 2)물리적 파일까지 한번에 삭제하는 법
