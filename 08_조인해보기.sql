...
-- employees 테이블의 부서 id 와 일치하는 departments 테이블의 부서 id 를
...
WHERE e.department_id = d.department_id; -- oracle JOIN 문법
-- 같다면 조회를 진행하겠다 
-- empoyees.department_id = departments.department_id
-- 조인조건을 만족하지 못하는애가 있어서 
-- 내가 조회하고자하는 , join 동일한고 있어야함 
-- 서로 다른 두 테이블을 합치는거 ! 
...
SELECT
    ...
    d.department_name, e.job_id, job_title, loc.city,
    loc.state_province
FROM 
    employees e, 
    departments d,
...
/*
1.loc테이블의 state_province = 'California' 조건에 맞는 값을 대상으로
...
일반조건이 붙은 테이블을 먼저 join 하고 
*/

-- 외부 조인
/*
상호 테이블간에 일치되는 값으로 연결되는 내부조인과는 다르게
...
-- departmentㅖ_id 는 다 출력하고 가져다 붙이는고 
-- 기준을 세워놓고
-- 회원만 글을 작성할수있는데 회원정보 = 게시물(글을 안썻을수도 있음,NULL)이때 외부조인 사용

/*
employees 테이블에는 존재하고, departments 테이블에는 존재하지 않아도
(+) 가 붙지 않은 테이블을 기준으로 하여 departments 테이블이 조인에
참여하라는 의미를 부여하기 위해 기호를 붙입니다. (outerjoin)
외부조인을 사용했더라도, 이후에 내부 조인을 사용하면
내부조인을 우선적으로 인식합니다.
*/

-- 외부 조인 진행시 모든 조건에 (+)를 붙여야하며
...
SELECT CROSS JOIN FROM employees;
