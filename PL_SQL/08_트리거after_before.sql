...
/*
    ...
    :OLD = 참조 전 열의 값 (INSERT: 입력 전 자료, UPDATE: 수정 전 자료, DELETE: 삭제될 값) 기존값
...
*/
-- 트리거는 rollback 이나 commit 안돔;
-- BEFORE 는 ROLLBACK  이 들어감 
...
CREATE TABLE tbl_user_backup(
    ...
    m_type VARCHAR2(10), --  변경타입 (업데이트가 발생된지, delate가 발생된건지 
...
BEGIN
    ...
    -- -> 원본 테이블에서 UPDATE or EELETE 된 사용자 정보 및 기타 정보)
    INSERT INTO tbl_user_backup 
    VALUES (:OLD.id, :OLD.name, :OLD.address, sysdate, v_type, USER());
      --  UESR() : 현재 로그인중인 계정 정보 (ex . hr)
END;

-- 확인!
INSERT INTO tbl_user('test01', 'kim', '서울');
INSERT INTO tbl_user('test02', 'lee', '경기');
INSERT INTO tbl_user('test03', 'honh', '부산');

SELECT * FROM tbl_user;
SELECT * FROM tbl_user_backup;

UPDATE tbl_user SET address = '인천' WHERE id = 'test01';
DELETE FORM tbl_user WHERE id = 'test02';
...
-- 상품 , 주문이 들어와야 상품이 깍이겠지 
CREATE TABLE product (
    product_no NUMBER(5) PRIMARY KEY,
    produch_name VARCHAR2(20),
    total NUMBER(5),
    price NUMBER(5)
);
...
BEGIN
    ...
    ELSIF v_total > v_product_total THEN-- 예외일부러발생
...
-- 트리거 내에서 예외가 발생하면 수행중인 INSERT 작업은 중단되며 ROLLBAKC이 진행됩니다.
